  ...
  Future<void> addFuel() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/api/fuel/store'),
        headers: headers,
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Fuel added successfully'),
            duration: Duration(seconds: 2),
          ),
        );
        Navigator.pop(context, true); // Pop page with success signal
      } else {
        print('Failed to add Fuel: ${response.statusCode}');
      }
    } catch (e) {
      print('Error adding Fuel: $e');
    } finally {
      setState(() {
        _isLoading = false;
      });
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: _isLoading ? null : addFuel,
