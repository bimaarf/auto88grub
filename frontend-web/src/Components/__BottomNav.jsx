...
export const BottomNav = ({
  ...
  return (
    <>
      <div className="w-full relative md:hidden z-50">
        <section
          id="bottom-navigation"
          className="md:hidden block fixed inset-x-0 bottom-0 z-10 bg-base-100 shadow">
          <section
            id="bottom-navigation"
            className="block fixed inset-x-0 bottom-0 z-10 bg-base-100 shadow">
            <div id="tabs" className="flex justify-between">
              <button
                onClick={() => navRedirect("/")}
                className="w-full focus:text-red-500 active:scale-95 duration-300 hover:text-red-500 text-pretty justify-center inline-block text-center pt-2 pb-1">
                <svg
                  width="25"
                  height="25"
                  viewBox="0 0 42 42"
              ...
              <button
                ...
                <span className="tab tab-kategori block text-xs">Menu</span>
              </button>
              <button className="w-full focus:text-red-500 active:scale-95 duration-300 hover:text-red-500 text-pretty justify-center inline-block text-center pt-2 pb-1">
                <label className="swap -mt-3 swap-rotate hover:bg-pink-600 hover:bg-opacity-25 rounded-xl p-1 scale-75 active:scale-90 duration-200">
                  <input
                  ...
                  {/* sun icon */}
                  <svg
                    className="swap-on fill-current w-10 h-10 inline-block"
                  ...
                  {/* moon icon */}
                  <svg
                    className="swap-off fill-current w-10 h-10 inline-block"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24">
                ...
                <span
                  className="tab tab-explore block text-xs"
                  style={{ marginTop: "-11px" }}>
                  Theme
                </span>
              </button>
              <button
                onClick={() => window.open(process.env.REACT_APP_API, "_blank")}
                className="w-full focus:text-red-500 active:scale-95 duration-300 hover:text-red-500 text-pretty justify-center inline-block text-center pt-2 pb-1">
                <i className="fas fa-user text-xl -mb-0.5"></i>
                <span className="tab tab-explore block text-xs -mb-0.5">
                  Login
                </span>
              </button>
            </div>
